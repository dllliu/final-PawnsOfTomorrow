...
# Brief Description
Our project is command-line chess, which will include different variants of chess that will be determined by the user typing in the mode they want to play. These variants are: single-player chess, double-player chess, 3-Check (first to check opponent three times wins), King of the Hill, (first one to get the king to the middle 4 squares wins), Chess960 (the positions of the pieces on the top and bottom ranks of the board are scrambled), Horde (one side only has pawns), Knight-Takeover(all knights on white side). If you are in the mood, there are also chess tactics where you try to figure out the best move(easypuzzles or hardpuzzles) and a demo(which will give you the solutions so you can quickly see our features). The pieces will move with commands like “c2 c3,” as opposed to a drag and drop alternative, which tells the program the initial and destination square.  A user can always resign by typing in “resign.”
