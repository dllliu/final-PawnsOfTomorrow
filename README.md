# Group Name: Pawns Of Tomorrow
# Group Member Names: Daniel Liu, Eugene Yoo
# Link To Prototype: https://docs.google.com/document/d/1bDvRKP7p-PgfG3VMVllPY6koIaWTCoqLTFEoWPa82vM/edit?usp=sharing

# Brief Description
Our project is command-line chess, which will include different variants of chess that will be determined by the user typing in the mode they want to play. These variants are: single-player chess, double-player chess, 3-Check (first to check opponent three times wins), King of the Hill, (first one to get the king to the middle 4 squares wins), Horde (one side only has pawns), Knight-Takeover(all knights on white side). If you are in the mood, there are also chess tactics where you try to figure out the best move(easypuzzles or hardpuzzles) and a demo(which will give you the solutions so you can quickly see our features). The pieces will move with commands like “c2 c3,” as opposed to a drag and drop alternative, which tells the program the initial and destination square.  A user can always resign by typing in “resign.”

# Compile/Run Instructions

You do not need to import any libraries to run our program. Set the terminal color to a good color where you can see the board clearly.
Compile and run ChessGame.java using java ChessGame. When prompted for a mode, here are the following modes that are accessible: If you’d like to play single player, type ‘solo’ to go against the computer. Double Player Chess is ‘double’ 3-Check Chess is ‘three-check’, King of the Hill Chess is 'king-hill,' Horde Chess is ‘horde.’  A new mode for a knight invasion is 'knight takeover.' To solve tactics, if you’d like to solve easy puzzles, type ‘easypuzzles,’ and if you’d like to solve hard puzzles, type ‘hardpuzzles.’ If you’d like to see the functionality of our program, type ‘demo.’ and follow the instructions for what moves you should do in the terminal output. As a side note, the difference between tactics and demo is that demo will explicty give you the move to type in. 
